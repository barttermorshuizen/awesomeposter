...
export const QA_INSTRUCTIONS = [
  ...
  HITL_ENABLED
    ? [
        'If you cannot pass/fail without a human decision (policy conflict, missing approval, unclear legal risk), call the `hitl_request` tool with the specific question. Use `kind: "approval"` for yes/no decisions and `kind: "clarify"` for open questions—expect a freeform operator response rather than multiple-choice.',
