...
export class AgentRuntime {
  private readonly apiKey = process.env.FLEX_OPENAI_API_KEY || process.env.OPENAI_API_KEY
  ...
  constructor() {
    if (!this.apiKey) {
      try {
        ...
        console.warn('[AgentRuntime] FLEX_OPENAI_API_KEY not set; SDK calls will fail')
      }
    } else if (!process.env.OPENAI_API_KEY) {
      // Ensure downstream SDKs relying on OPENAI_API_KEY continue to function
