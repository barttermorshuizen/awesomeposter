...
1. Provide an authenticated POST endpoint (e.g., `POST /api/discovery/config-suggestions`) accepting `{ url: string, options?: { pagination?: boolean } }` and returning candidate `webList` selectors: `list_container_selector`, `item_selector`, field mappings (`title`, `excerpt`, `url`, `timestamp`), and optional pagination hint (`next_page`).
2. Response payload includes a JSON snippet ready for insertion into a source configuration plus a `metadata` object with confidence levels, warnings, and notes on multiple candidates when applicable.
...
- Unit-test selector heuristics and schema validation within `packages/shared/src/discovery/__tests__/config-suggestions.spec.ts`, including DOM samples with multiple list patterns, pagination hints, and invalid selectors.
