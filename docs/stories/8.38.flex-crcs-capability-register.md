...
## Acceptance Criteria
1. `PlannerService` computes a Comprehensive Relevant Capability Set (CRCS) before every `plan_requested` event by building the facet–capability graph for the current TaskEnvelope: only capabilities that lie on at least one valid path from an available input facet (envelope inputs, run-context facets, or provided hints) to a required output or `goal_condition` facet are included. Policy- or goal-condition-pinned capabilities are annotated but still must live on a valid path; disconnected capabilities are excluded and surfaced via `missingPinnedCapabilityIds`. Each included capability is tagged with `reasonCodes` (`path`, `policy_reference`, `goal_condition`) that justify its inclusion. (`docs/architecture/flex-agents-server/5-core-concepts.md#51-taskenvelope`, `docs/architecture/flex-agents-server/5-core-concepts.md#plannerpolicy`, `docs/architecture/flex-agents-server/5-core-concepts.md#facet-aligned-capabilities`, `docs/architecture/flex-agents-server/11-capability-registry-agent-contracts.md#11-capability-registry-agent-contracts`)
2. The planner prompt’s `CAPABILITY REGISTRY SUMMARY` section renders only the CRCS table (registry order preserved), listing `capabilityId`, display name, facet coverage, and `reasonCodes`. A configurable cap keeps this table within the semantic contract blueprint budget while telemetry (`flex.planner.prompt.*`) records prompt token counts improving by ≥20% compared to the pre-CRCS baseline. (`docs/architecture/flex-agents-server/5-core-concepts.md#planner-prompt-blueprint-v2`, `docs/architecture/flex-agents-server/6-component-responsibilities.md#6-component-responsibilities`, `docs/architecture/tech-stack.md#1-core-runtime-components`)
...
4. Capabilities referenced by planner/runtime policies, routing rules, or `goal_condition` repairs must also live on a valid input→output path; if they are disconnected, the CRCS payload reports them via `missingPinnedCapabilityIds` so `PolicyNormalizer`, `PlannerValidationService`, and `ContextBuilder` can halt the run and request replans or operator escalation. (`docs/architecture/flex-agents-server/5-core-concepts.md#plannerpolicy`, `docs/architecture/flex-agents-server/5-core-concepts.md#interaction-sequence`, `docs/architecture/flex-agents-server/6-component-responsibilities.md#6-component-responsibilities`)
5. Automated coverage (unit + integration) proves the reachability graph logic produces the expected set, that the prompt table ordering and SSE payloads reflect the computed metadata, and that planner prompt size reductions do not change `plan_rejected` rates. (`docs/architecture/coding-standards.md#7-testing-expectations`, `docs/architecture/discovery-agent-backend.md#testing-strategy`)
...
### Summary
- Verified CRCS policy pinning is now limited to `selection.require` + runtime selectors, so `avoid/forbid` preferences no longer inflate `missingPinnedCapabilityIds`.
- Confirmed planner halts immediately when CRCS reports missing pinned capabilities and the coordinator emits a `plan_rejected` frame before surfacing the model.
- Targeted Vitest suites: `npm run test:unit -- packages/flex-agents-server/__tests__/planner-service.crcs.spec.ts packages/flex-agents-server/__tests__/planner-service.prompts.spec.ts packages/flex-agents-server/__tests__/telemetry-crcs.spec.ts packages/flex-agents-server/__tests__/flex-planner.crcs.spec.ts` ✅

### Findings
1. **Resolved – Pinned capability enforcement**: `FlexPlanner.buildPlan` now throws `MissingPinnedCapabilitiesError` when `crcs.missingPinnedCapabilityIds` is populated, and the run coordinator emits `plan_rejected` with `reason: missing_pinned_capabilities` before aborting. AC4 is satisfied.
2. **Resolved – Selection buckets**: `collectPinnedCapabilities` only includes `selection.require` entries as hard pins, so avoid/forbid preferences no longer create false errors. Coverage added in `planner-service.crcs.spec.ts`.

### Recommendation
- Status recommendation: **Ready for Done** – CRCS enforcement behavior and diagnostics now align with AC4 plus telemetry expectations.
...
### Summary
- Verified CRCS computation, planner prompt wiring, telemetry, and SSE payloads across `flex-capability-registry`, `planner-service`, `flex-planner`, and `flex-run-coordinator`.
- Validated documentation shards (`docs/architecture/flex-agents-server/{5,10,11}`) and shared contracts (`packages/shared/src/flex/types.ts`) to confirm schema alignment.
- Targeted Vitest suites: `npm run test:unit -- packages/flex-agents-server/__tests__/planner-service.crcs.spec.ts packages/flex-agents-server/__tests__/planner-service.prompts.spec.ts packages/flex-agents-server/__tests__/telemetry-crcs.spec.ts` ✅ (initial attempt with `--runInBand` failed because Vitest does not recognize that flag).

### Findings
1. **HIGH – Missing enforcement when pinned capabilities are disconnected (AC4).** `computeCrcsSnapshot` surfaces `missingPinnedCapabilityIds`, but the orchestration flow only logs/telemeters this data (`packages/flex-agents-server/src/services/flex-run-coordinator.ts:913`, `:1714`, `packages/flex-agents-server/src/services/flex-planner.ts:561-612`) and never halts the run, raises validation errors, or triggers replans when required capabilities are unreachable. Acceptance Criterion 4 explicitly requires `PolicyNormalizer`, `PlannerValidationService`, or `ContextBuilder` to stop execution or escalate before proceeding, so the run can still continue with an invalid plan.
2. **MEDIUM – Planner selection `avoid`/`forbid` incorrectly treated as “pinned”.** `collectPinnedCapabilities` adds *all* selection buckets to `pinnedFromPolicies` (`packages/flex-agents-server/src/services/flex-capability-registry.ts:615-635`), which flags capabilities the policy explicitly says to avoid/forbid as “missing pinned” when they are intentionally excluded. This inflates telemetry noise and may mask genuine policy or goal-condition gaps because every “avoid” entry now shows up as a CRCS error.

### Recommendation
- Status recommendation: **Changes Required** until pinned-capability enforcement blocks/escapes runs and only genuine “required” pins contribute to CRCS gap tracking.
