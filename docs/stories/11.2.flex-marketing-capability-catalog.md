...
- Updated `/api/v1/flex/sandbox/metadata` to always serve the marketing snapshot and removed the `FLEX_SANDBOX_LEGACY_CATALOG` fallback toggle.
- Documented rollout/rollback guidance in `docs/architecture/flex-agents-server/11-capability-registry-agent-contracts.md`.
- Risk: sandbox now depends on the marketing facet tag filter—Story 11.3 should verify no downstream consumers rely on legacy facet IDs after the fallback flag removal.

### File List
- packages/shared/src/flex/marketing-catalog.ts
- packages/shared/src/flex/facets/catalog.ts
- packages/shared/src/index.ts
- packages/flex-agents-server/src/agents/runtime-capabilities.ts
- packages/flex-agents-server/src/services/agents-container.ts
...
- packages/flex-agents-server/__tests__/tools-policy.spec.ts
