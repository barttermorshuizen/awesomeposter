...
## Tasks / Subtasks
- [x] Extend policy normalization to detect DSL strings, compile them with the shared parser, and persist JSON-Logic plus DSL metadata. (AC 1,3)
- [x] Update Nitro validation endpoints to invoke `validateConditionInput` for DSL submissions and surface consistent error payloads. (AC 2)
- [x] Wire Flex Sandbox policy forms to persist DSL + JSON, migrate legacy policies on load, and respect the DSL feature flag. (AC 4)
- [x] Add regression tests covering DSL normalization, runtime trigger execution, and representative quantifier expressions. (AC 5)
- [x] Refresh architecture and authoring docs to describe DSL usage in runtime policies. (AC 1,2,4)
...
- [x] Tests added and passing (unit, integration, UI as applicable)
- [x] Documentation updated for runtime policy DSL usage
- [x] Feature flag strategy validated for rollout/fallback
...
- `pnpm vitest run packages/shared/__tests__/condition-dsl.spec.ts packages/flex-agents-server/__tests__/policies/policy-normalizer.spec.ts server/utils/__tests__/condition-dsl-helper.spec.ts src/views/__tests__/FlexSandboxView.spec.ts packages/flex-agents-server/__tests__/flex-run-dsl.spec.ts`

### Completion Notes List
- Added facet-driven `dslPath` alias generation so DSL authors can reference `facets.*` paths while JSON-Logic persists canonical run-context variables.
- Normalized runtime policy ingestion (policy normalizer + `flex/run` route) to compile DSL strings via the shared validator and capture warnings, canonical DSL, and variable metadata.
- Updated Flex Sandbox DSL editor, feature flag handling, and documentation/help content to round-trip the new aliases and explain the normalization contract.
...
## QA Results
- **Decision:** PASS – runtime policies now accept DSL strings end-to-end (normalizer, validation route, and evaluation) with UI authoring parity under the existing feature flag.
- **Validation:** `pnpm vitest run packages/shared/__tests__/condition-dsl.spec.ts packages/flex-agents-server/__tests__/policies/policy-normalizer.spec.ts packages/flex-agents-server/__tests__/flex-run-dsl.spec.ts server/utils/__tests__/condition-dsl-helper.spec.ts src/views/__tests__/FlexSandboxView.spec.ts`
- **Risks:** Helper duplication in `server/utils/condition-dsl.ts` and `packages/flex-agents-server/src/utils/condition-dsl.ts` could drift; consider centralising via shared export.
