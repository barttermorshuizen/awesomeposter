...
**Dependencies:** Story 8.9 (TaskPolicies schema foundation) must be complete; coordinate with Story 8.23 (action execution) for shared metadata when planner annotations reference runtime behavior. Keep Story 8.24 aligned so conditional routing nodes reuse the canonical action definitions.
...
- 2025-10-22: `npm run test:unit` (all suites passed locally).
...
- Envelope hints now derive entirely from TaskEnvelope and planner policy data; planner metadata persists those hints for telemetry (`packages/flex-agents-server/src/services/flex-planner.ts:288`, `packages/flex-agents-server/src/services/flex-planner.ts:447`).
- PlannerService prompts integrate structured context hints and canonical policies without scenario tags (`packages/flex-agents-server/src/services/planner-service.ts:204`).
- FlexRunCoordinator streams planner context in plan_requested frames so SSE consumers stay scenario-free (`packages/flex-agents-server/src/services/flex-run-coordinator.ts:245`).
- Architecture references highlight envelope-driven guidance for future contributors (`docs/flex-agentic-architecture201025.md:71`).

### Refactoring Performed
- None.

### Compliance Check
- Coding Standards: ✓ Aligned with TypeScript/Vitest patterns; regression suite passes (`npm run test:unit`).
- Project Structure: ✓ All updates live under documented planner services plus supporting docs.
- Testing Strategy: ✓ Planner regression spec exercises LinkedIn/blog/email envelopes and assertions on planner context (`packages/flex-agents-server/__tests__/flex-planner.spec.ts:200`).
- All ACs Met: ✓ AC1–AC4 satisfied via code, tests, and docs; story header still reads “Ready for dev”—update to “Ready for Review” before closure.

### Improvements Checklist
- [ ] Add sandbox integration sample covering multi-language envelopes to assert planner context languages stream correctly (`packages/flex-agents-server/__tests__/flex-sandbox-run.spec.ts`).
- [ ] Capture fallback regression ensuring variantCount values above six clamp as expected in planner metadata.
- [ ] Document deterministic fallback capability assumption in the flex runbook for operational awareness.

### Security Review
- No new threat vectors; planner now consumes validated envelope/policy payloads instead of hard-coded marketing scenarios.

### Performance Considerations
- derivePlannerContext operates on in-memory envelope data only; no additional planner requests introduced.

### Files Modified During Review
- None.

### Gate Status
Gate: PASS → docs/qa/gates/8.25-flex-envelope-driven-planner.yml
Risk profile: docs/qa/assessments/8.25-risk-20251022.md (not generated)
NFR assessment: docs/qa/assessments/8.25-nfr-20251022.md (not generated)
