...
- npx vitest run server/utils/__tests__/discovery-repository.spec.ts src/__tests__/ClientDiscoverySourcesPanel.spec.ts src/__tests__/ClientDiscoveryKeywordsPanel.spec.ts
...
- Hardened duplicate detection with a case-insensitive unique index and repository guard, plus regression coverage for concurrent mixed-case submissions.
- Replaced discovery source failure alerts with the shared toast notifications stack and introduced global notification UI plumbing.
- Hooked discovery SSE consumers to `feature_disabled` events so the UI rolls back immediately when the flag drops, with component coverage for the disable path.

### File List
- docs/discovery-agent-pilot-onboarding.md
- packages/db/drizzle/0009_discovery_sources.sql
- packages/db/drizzle/0011_discovery_sources_identifier_lower_idx.sql
...
- src/components/clients/ClientDiscoveryKeywordsPanel.vue
...
- `packages/db/drizzle/0011_discovery_sources_identifier_lower_idx.sql:1`, `packages/db/src/schema.ts:205`, `server/utils/discovery-repository.ts:126` – Lower(identifier) unique index with legacy dedupe enforces case-insensitive duplicates at the database boundary and repository layer.
- `server/api/events/discovery.get.ts:154`, `src/lib/discovery-sse.ts:102`, `src/components/clients/ClientDiscoverySourcesPanel.vue:206` – SSE emits `feature_disabled` and the client now tears down the stream, disables the form, and surfaces the disable message immediately.

### Refactoring Performed
- `packages/db/drizzle/0011_discovery_sources_identifier_lower_idx.sql:1` removes pre-existing case-variant rows before establishing the lower(identifier) unique index.

### Compliance Check
- Coding Standards: ✓ Naming/Drizzle conventions remain consistent across discovery schema updates
- Project Structure: ✓ Migration + schema changes colocated with discovery data model assets
- Testing Strategy: ✓ `tests/api/discovery/events-stream.spec.ts:360`, `server/utils/__tests__/discovery-repository.spec.ts:128`, `src/__tests__/ClientDiscoverySourcesPanel.spec.ts:79`, `src/__tests__/ClientDiscoveryKeywordsPanel.spec.ts:19`
- All ACs Met: ✓ AC1–AC5 covered after duplicate guard and SSE rollback handling

### Improvements Checklist
- [x] Persist duplicate keys (or index on lower(identifier)) before insert so mixed-case duplicates are rejected atomically (`packages/db/drizzle/0011_discovery_sources_identifier_lower_idx.sql:1`, `packages/db/src/schema.ts:205`, `server/utils/discovery-repository.ts:139`)
- [x] Add SSE client handling (and tests) for `feature_disabled` notifications to trigger UI rollback when discovery is shut off (`server/api/events/discovery.get.ts:154`, `src/lib/discovery-sse.ts:102`, `src/components/clients/ClientDiscoverySourcesPanel.vue:206`)
- [x] Backfill automated coverage for the above scenarios (integration + component tests per story testing section) (`server/utils/__tests__/discovery-repository.spec.ts:128`, `tests/api/discovery/events-stream.spec.ts:360`, `src/__tests__/ClientDiscoverySourcesPanel.spec.ts:79`, `src/__tests__/ClientDiscoveryKeywordsPanel.spec.ts:19`)

### Security Review
- PASS – No additional surface beyond validated discovery endpoints

### Performance Considerations
- PASS – Lower(identifier) index + SSE client cleanup have negligible overhead

### Files Modified During Review
- None (verification only)

### Gate Status
Gate: PASS → docs/qa/gates/2.1-client-source-add-http-source.yml
Risk profile: _not generated_
NFR assessment: _not generated_

### Recommended Status
✓ Ready for Done – QA gate PASS with AC coverage and regression protections confirmed
