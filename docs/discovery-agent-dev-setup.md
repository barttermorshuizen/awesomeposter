...
- **Telemetry & Observability**: each run emits `ingestion.started` and `ingestion.completed` events (SSE + logs) with `runId`, `sourceId`, `sourceType`, duration, success flag, and optional `failure_reason`/`retry_in_minutes`. Use these events to confirm RSS cadences (expect steady 60-minute spacing) and YouTube cadence adjustments (longer intervals during quota scarcity).
- **Failure Handling**: structured telemetry captures `network_error`, `http_4xx`, `http_5xx`, `youtube_quota`, and more. When monitoring, investigate repeated `youtube_quota` within a 24h window—lower cadence or rotate credentials. RSS failures with `http_5xx` automatically schedule quick retries; coordinate with Story 3.3 for sustained backoff policies.
