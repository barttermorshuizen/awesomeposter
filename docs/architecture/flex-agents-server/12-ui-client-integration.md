...
- Sandbox plan inspector surfaces capability post-condition guards/results from streamed `plan_generated/plan_updated/node_complete/goal_condition_failed/complete` frames, rendering pass/pending/fail chips with facet/path tooltips keyed to node + capability.
- Flex Sandbox run-level views add a guard health summary (counts + latest failing facet) beside the timeline/HITL flows when `USE_FLEX_DEV_SANDBOX` is enabled, degrading gracefully to an informational banner when no post conditions are present.
