...
- `plan_generated`: contains trimmed plan metadata (`nodes[{ id, capabilityId, label }]`). Each node now carries `postConditionGuards[{ facet, paths[] }]` summarising the capability-level predicates enforced by the planner, plus the latest `postConditionResults[]` when resumed from persistence. Replan-driven generations echo the `replan` metadata described above so clients can display rationale alongside the fresh plan version, and include the most recent `crcs` summary so UIs can compare planner prompt density before/after the change.
- `plan_updated`: new plan version persisted after replanning or HITL resume; payload includes `previousVersion`, `version`, summary node statuses, trigger metadata, and (when applicable) the same `replan` structure so operators can see why the plan changed. Plan updates reuse the `postConditionGuards` / `postConditionResults` metadata so dashboards can reason about which capabilities remain guarded after each revision. Each update replays the latest `crcs` summary so dashboards can correlate plan churn with registry breadth.
- `node_start` / `node_complete` / `node_error`: per-node execution lifecycle. `node_start` frames surface the resolved `postConditionGuards` for the targeted capability while `node_complete` frames append `postConditionResults[{ facet, path, satisfied, error? }]` so downstream subscribers never have to re-run predicate evaluation.
- `policy_triggered`: emitted when runtime policies fire (including during resume); payload includes canonical `actionDetails` (type, metadata, nested follow-ups) alongside legacy fields so clients can identify the requested behaviour. When the trigger is `onPostConditionFailed` the payload also includes `attempt`, `maxRetries`, and the evaluated `postConditionResults[]` so operators can tell whether the engine is still retrying a guarded capability or executing the terminal action.
...
- `complete`: final frame containing `payload.output` that satisfies the caller schema. When envelopes supplied `goal_condition` entries, `payload.goal_condition_results` lists the evaluated predicates (facet, path, expression, DSL, JSON-Logic, observed value, satisfied flag, error message if evaluation failed) so clients can see which ones passed without re-inspecting facet snapshots. When capabilities emitted post-condition metadata, `payload.post_condition_results` aggregates `{ nodeId, capabilityId, results[] }` for every guarded node so UIs can review per-capability predicate outcomes alongside the final output. The orchestrator withholds this frame until all failed predicates have been reparsed and satisfied or the automatic replan cap has been exceeded.
