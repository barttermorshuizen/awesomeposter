...
const posts = pgTable('posts', {
    ...
    knobPayloadJson: jsonb('knob_payload_json').$type(), // New field for knob payload
    status: text('status'),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow()
});
...
const postTelemetry = pgTable('post_telemetry', {
    ...
    postId: uuid('post_id').references(() => posts.id, { onDelete: 'cascade' }),
    clientId: uuid('client_id').references(() => clients.id, { onDelete: 'cascade' }),
    knobsJson: jsonb('knobs_json').$type(), // Full knob payload
    observablesJson: jsonb('observables_json').$type(), // Raw metrics
    derivedMetricsJson: jsonb('derived_metrics_json').$type(), // Calculated metrics
    renderMetricsJson: jsonb('render_metrics_json').$type(), // Content analysis
...
export { assets as a, briefs as b, clients as c, clientFeatures as d, clientFeatureToggleAudits as e, emailsIngested as f, getDb as g, examplesIndex as h, getPool as i, discoveryItems as j, discoveryScores as k, discoveryKeywords as l, discoverySources as m, discoveryIngestRuns as n, clientProfiles as o, tasks as t };
